...
#writing the data to csv file again
#write.csv(test,"submit.csv")
submission<-c('Item_Identifier','Outlet_Identifier','Sale_preds_reg')
submit <- test[submission]
write.csv(submit, 'submissions.csv', row.names = TRUE)

#submit<-c["Item_Identifier","Outlet_Identifier",""]